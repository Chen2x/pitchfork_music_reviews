...
avgreviewer = pd.read_sql_query("SELECT reviewid, author, AVG(score) as avgscore, COUNT(author) as articles FROM reviews GROUP BY author HAVING COUNT(author)>2 ORDER BY AVG(score) DESC LIMIT 10", conn)
